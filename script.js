...
Date.prototype.sToTime=function(value){
    this.value=value
...
let todayArray = wordBank[time.getDOY()%wordBank.length].split("");
let todayWord = wordBank[time.getDOY()%wordBank.length]
document.getElementById("time").innerHTML=time.sToTime((24*60*60) - (time.getHours()*60*60) - (time.getMinutes()*60) - time.getSeconds())+" Until Next Word"
...
function submit(){
    ...
    for (let i=1; i<6; i++){
        ...
        if (document.getElementById(curId).innerText.trim()!==""){
            guessWord.push(document.getElementById(curId).innerText);
        }
        else{
            break;
        }
    ...
    if (guessWord.length==5){
        if (wordBank.includes(guessWord.join("")) || allWordBank.includes(guessWord.join("").toLowerCase())){
...
function storeToLocalStorage(){
    localStorage.setItem("grid", document.querySelector("table").innerHTML)
    localStorage.setItem("row", row)
}

function useFromLocalStorage(){
    if (localStorage.getItem("grid")!==null){
        document.querySelector("table").innerHTML=localStorage.getItem("grid")
...
function checkCorrect(currentGuess){
    ...
    let tempTodayArray=todayArray.slice()
    for (var i = 0; i < 5; i++){
        ...
        if(currentGuess[i] == todayArray[i]){
            ...
            tempTodayArray.splice(i,1,"_")
            howManyCorrect+=1
            console.log(tempTodayArray)
        }
        else if(tempTodayArray.includes(currentGuess[i])){
            let j=0
            ...
            for (j = 0; j < 5; j++){
                ...
                console.log(currentGuess[i])
                console.log("todayArray J")
                console.log(todayArray[j])
                if (currentGuess[i]==todayArray[j] && currentGuess[j]!=todayArray[j]){
                    noDuplicates=true
                    break
                }
            }
            if (noDuplicates){
                ...
                if (document.getElementById(currentGuess[i]).style.backgroundColor!="green" && document.getElementById(currentGuess[i]).style.backgroundColor!="yellow" && document.getElementById(currentGuess[i]).style.backgroundColor!="grey"){
                    document.getElementById(currentGuess[i]).style.backgroundColor="yellow"
                }
                tempTodayArray.splice(j,1,"_")
                console.log(tempTodayArray)
            }
            else{
...
function signUpScreen(){
    var createAccount = `
    <fieldset style="display: inline-block; background-color: white; border-radius: 5px; border-color: Black;border-width: thick ">
    <span class = 'newClass'>⤫</span>

    <h1 style = "text-align: center;">How To Play</h1> 
        Guess the <b>CODLE</b> in six tries.
        <br>
        <br>
        ...
        EXAMPLES
        <br>
        <img src="Example1.png" alt="Correct letter" height = "60px" width= "250">
        <br>
        The letter A is in the word and in the correct spot.
        <br>
        <br>
        <img src="Example2.png" alt="Misplaced letter" height = "60px" width= "250">
        <br>
        The letter G is in the word but in the wrong spot.
        <br>
        <br>
        <img src="Example3.png" alt="Incorrect letter" height = "60px" width= "250">
        <br>
        ...
        -------------------------------------------------------------------------------
        <br>
        A new <b>CODLE</b> will be available each day!

        `

    document.getElementById('overlay').innerHTML = createAccount;
    document.getElementById("overlay").style.display = "block";
    
}

function displayOutputWin(){
    var output = "";
    ...
    if (row == 1){
        ...
        `           
        //<input type="button" value="Random Word" onclick="randomWord()" class="resetWord">
...
function displayOutputLose(){
    var output = "";
        output +=`
        <br><br><br><br><br><br><br>
        <fieldset style="display: inline-block; background-color: white; border-radius: 10px; border-color: Black; border-width: thick ">
        <h1>The Correct Answer Was</h1>
        <b><i>${todayWord}</i></b><br><br>
        Come Back Tommorrow!
        `    
document.getElementById('overlay').innerHTML = output;
document.getElementById("overlay").style.display = "block";
...
function randomWord(){
    todayWord = wordBank[randomInt(0,wordBank.length)]
    todayArray = todayWord.split("");
    
    //I is row
    //J is coloumn
    for (let row = 1; row < 7; row++){
        for (var j = 0; j < 5; j++){
            const curId = String(row) + String(j+1);
