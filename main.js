...
//Timer till next day. Modify it to change the thing it counts down too.
function countDownToNextDay(element){
    const myInterval = setInterval(() => {
        const time=new Date
        document.querySelector(element).innerHTML=time.sToTime((24*60*60) - (time.getHours()*60*60) - (time.getMinutes()*60) - time.getSeconds())+" Until Next Word"
    }, 1000);
}
